...
fifo_ret_t fifo_read(fifo_t *fifo, fifo_data_t *p_out, size_t len) {
  ...
  if (fifo->head > fifo->tail) {
    ...
    if ((len + fifo->tail) > fifo->len) {
      memcpy(p_out, &fifo->buffer[fifo->tail],
             (fifo->len - fifo->tail) * sizeof(fifo_data_t));
      memcpy(&p_out[fifo->len - fifo->tail], &fifo->buffer[0],
             (len - (fifo->len - fifo->tail)) * sizeof(fifo_data_t));
...
fifo_ret_t fifo_write(fifo_t *fifo, fifo_data_t *p_in, size_t len) {
  ...
  if (len + fifo->head > fifo->len) {
    memcpy(&fifo->buffer[fifo->head], p_in,
           (fifo->len - fifo->head) * sizeof(fifo_data_t));
    memcpy(fifo->buffer, &p_in[fifo->len - fifo->head],
           (len - (fifo->len - fifo->head)) * sizeof(fifo_data_t));
